...
for (i in 1:2) { # could actually find the subdirectories, but whatever
  ...
  for (runset_num in 1:4) {
    ...
    set_rslts[[listkey]] <- process_one_found_set_of_cases(model_versionyear, model_subversion, runset_num)
  
    if (save_results_to_CSV) {
      write_csv(set_rslts[[listkey]], 
