...
runset_num = 3
case_file_firstnames = paste0(model_versionyear, model_subversion,
...
case_example %in% case_file_firstnames
```
...
rslt <- here(
  ...
  paste0(case_file_firstnames[run_num],
         # "SN", run_num,
         ".xlsx")
  ) %>%
  read_excel()
...
process_one_found_set_of_cases <- function(mod_versionyear, mod_subversion, runset_n) {
  ...
  print(paste0("Processing `model_versionyear` ", mod_versionyear, 
...
```{r}
runset_num <- 4
set_rslts <- process_one_found_set_of_cases(model_versionyear, model_subversion, runset_num)
```
